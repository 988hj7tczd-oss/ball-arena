...
globalThis.Platform = {
    ...
    onTouchStart(c, cb) {
        tt.onTouchStart(e => { cb({ changedTouches: (e.changedTouches||[]).map(t => ({clientX:t.clientX||t.x||t.pageX,clientY:t.clientY||t.y||t.pageY,identifier:t.identifier||t.id})) }); });
    },
    onTouchMove(c, cb) {
        tt.onTouchMove(e => { cb({ changedTouches: (e.changedTouches||[]).map(t => ({clientX:t.clientX||t.x||t.pageX,clientY:t.clientY||t.y||t.pageY,identifier:t.identifier||t.id})) }); });
    },
    onTouchEnd(c, cb) {
        tt.onTouchEnd(e => { cb({ changedTouches: (e.changedTouches||[]).map(t => ({clientX:t.clientX||t.x||t.pageX,clientY:t.clientY||t.y||t.pageY,identifier:t.identifier||t.id})) }); });
    },
    onTouchCancel(c, cb) {
    ...
    // ─── 本地存储 ───
    getItem(key) {
        try { return tt.getStorageSync(key); } catch (e) { return null; }
    },
    setItem(key, value) {
        try { tt.setStorageSync(key, value); } catch (e) {}
    },
